<div class="data-kelas-7">
    ...
    <table class="tabel-data jurnal-kelas-table">
        ...
        @foreach ($kelas9->reverse() as $data)
        <tr>
            @foreach ($data->jurnal_kelas->where('kelas', '9') as $h )
        <tr>
            <td></td>
            <td>{{$data->name}}</td>
            <td>
    ...
    <div class="signature d-flex mt-5 px-5 align-items-center py-2">
        ...
        <div class="w-50 d-flex justify-content-end px-5 align-items-center">
            <div class="text-center">
                <span>Wali Kelas 9</span><br><br><br><br><br>
                @foreach($kepala->where('kelas', '=', '9') as $data)
                <span>{{$data->nama}}</span><br>
                <span>NIPY. {{$data->NIPY}}</span>
                @endforeach
            </div>
